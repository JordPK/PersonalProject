Shader "Unlit/LightBlueTriPlanarMatrix"
{
    Properties
    {
    ...
    SubShader
    {
         ...
         Pass
         {
             ...
             {
                 float2 uv    = frac (coord.xy/ 16.);                // Geting the fract part of the block, this is the uv map for the block
                 float2 block = floor(coord.xy/ 16.);                // Getting the id for the block. The first block is (0,0) to its right (1,0), and above it (0,1) 
         ...
         float3 rain_colored(float2 fragCoord)
{
    ...
    float offset = rnd(fragCoord.x * 521., 612);           
    float speed = rnd(fragCoord.x * 612., 951) * .15 + .35; 
    speed *= 0.4;
    ...
    // Modify the color here to light blue
    float3 col = float3(0.2, .7, 1.0); // Light blue color
